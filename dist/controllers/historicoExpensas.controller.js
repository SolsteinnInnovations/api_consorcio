...
exports.eliminarHistoricoExpensa = exports.actualizarHistoricoExpensa = exports.getHistoricoExpensaById = exports.getHistoricoExpensas = exports.crearHistoricoExpensa = void 0;
const historicoExpensas_model_1 = require("../models/historicoExpensas.model");
...
const crearHistoricoExpensa = async (req, res) => {
    const { idExpensa, idDepartamento, ordinarias, extraordinarias } = req.body;
    try {
        // Validaciones habilitadas
        const existeExpensa = await expensas_model_1.ExpensasModel.findById(idExpensa);
        if (!existeExpensa) {
        ...
        const historicoExpensa = new historicoExpensas_model_1.HistoricoExpensasModel(req.body);
...
exports.crearHistoricoExpensa = crearHistoricoExpensa;
const getHistoricoExpensas = async (req, res) => {
    try {
        const historicoExpensas = await historicoExpensas_model_1.HistoricoExpensasModel.find()
...
exports.getHistoricoExpensas = getHistoricoExpensas;
const getHistoricoExpensaById = async (req, res) => {
    const id = req.params.id;
    try {
        const historicoExpensa = await historicoExpensas_model_1.HistoricoExpensasModel.findById(id)
            .populate('idExpensa', 'periodo fechaVencimiento ordinarias extraordinarias estado') // Se seleccionan campos relevantes de Expensa
            .populate('idDepartamento', 'identificacion piso');
        if (!historicoExpensa) {
...
exports.getHistoricoExpensaById = getHistoricoExpensaById;
...
const actualizarHistoricoExpensa = async (req, res) => {
    const id = req.params.id;
    ...
    try {
        const historicoExpensaDB = await historicoExpensas_model_1.HistoricoExpensasModel.findById(id);
        ...
        const historicoExpensaActualizado = await historicoExpensas_model_1.HistoricoExpensasModel.findByIdAndUpdate(id, campos, { new: true });
        res.status(200).json({
            ok: true,
...
exports.actualizarHistoricoExpensa = actualizarHistoricoExpensa;
const eliminarHistoricoExpensa = async (req, res) => {
    const id = req.params.id;
    try {
        const historicoExpensaBorrado = await historicoExpensas_model_1.HistoricoExpensasModel.findByIdAndDelete(id);
