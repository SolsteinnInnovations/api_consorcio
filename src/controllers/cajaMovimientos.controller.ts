...
import { CajaMovimientosModel, ICajaMovimiento } from '../models/cajaMovimientos.model';
import { ReclamosModel } from '../models/reclamos.model';
import { TipoMovimientosModel } from '../models/tipoMovimientos.model';
import { EdificiosModel } from '../models/edificios.model';

const crearCajaMovimiento = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const existeTipoMovimiento = await TipoMovimientosModel.findById(idTipoMovimiento);
        ...
        const ultimoMovimiento = await CajaMovimientosModel.findOne({ idEdificio })
        ...
        const cajaMovimiento = new CajaMovimientosModel(movimientoData);
...
const getCajaMovimientos = async (req: Request, res: Response) => {
    try {
        const cajaMovimientos = await CajaMovimientosModel.find()
...
const getCajaMovimientoById = async (req: Request, res: Response) => {
    const id = req.params.id;
    try {
        const cajaMovimiento = await CajaMovimientosModel.findById(id)
            .populate('idReclamo', 'identificacion titulo')
            .populate('idTipoMovimiento', 'descripcion')
            .populate('idEdificio', 'direccion identificadorEdificio');
        if (!cajaMovimiento) {
...
const actualizarCajaMovimiento = async (req: Request, res: Response) => {
    const id = req.params.id;
    const { idReclamo, idTipoMovimiento, idEdificio, ...campos } = req.body;
    try {
        const cajaMovimientoDB = await CajaMovimientosModel.findById(id);
        if (!cajaMovimientoDB) {
            return res.status(404).json({
                ok: false,
                msg: 'Movimiento de caja no encontrado por ID.'
        ...
        const cajaMovimientoActualizado = await CajaMovimientosModel.findByIdAndUpdate(id, campos, { new: true });
...
const eliminarCajaMovimiento = async (req: Request, res: Response) => {
    const id = req.params.id;
    try {
        const cajaMovimientoBorrado = await CajaMovimientosModel.findByIdAndDelete(id);
...
const getSaldoCajaByEdificio = async (req: Request, res: Response) => {
