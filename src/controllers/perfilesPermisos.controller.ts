...
import { PerfilesPermisosModel, IPerfilPermiso } from '../models/perfilesPermisos.model';
import { PerfilesModel } from '../models/perfiles.model'; // Para validar idPerfil
import { PermisosModel } from '../models/permisos.model';   // Para validar idPermiso

const crearPerfilPermiso = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const existePermiso = await PermisosModel.findById(idPermiso);
        if (!existePermiso) {
            return res.status(400).json({
                ok: false,
        ...
        const existePerfilPermiso = await PerfilesPermisosModel.findOne({ idPerfil, idPermiso });
        ...
        const perfilPermiso = new PerfilesPermisosModel(req.body);
...
const getPerfilesPermisos = async (req: Request, res: Response) => {
    try {
        // Popula tanto el perfil como el permiso, y del permiso también el idItemMenu
        const perfilesPermisos = await PerfilesPermisosModel.find()
...
const getPerfilPermisoById = async (req: Request, res: Response) => {
    const id = req.params.id;
    try {
        const perfilPermiso = await PerfilesPermisosModel.findById(id)
            .populate('idPerfil', 'descripcion')
            .populate({
                path: 'idPermiso',
...
const actualizarPerfilPermiso = async (req: Request, res: Response) => {
    const id = req.params.id;
    const { idPerfil, idPermiso, ...campos } = req.body;
    try {
        const perfilPermisoDB = await PerfilesPermisosModel.findById(id);
        if (!perfilPermisoDB) {
            return res.status(404).json({
                ok: false,
                msg: 'Asignación de permiso a perfil no encontrada por ID.'
        ...
        const perfilPermisoActualizado = await PerfilesPermisosModel.findByIdAndUpdate(id, campos, { new: true });
        res.status(200).json({
            ok: true,
...
const eliminarPerfilPermiso = async (req: Request, res: Response) => {
    const id = req.params.id;
    try {
        const perfilPermisoDB = await PerfilesPermisosModel.findById(id);
        if (!perfilPermisoDB) {
