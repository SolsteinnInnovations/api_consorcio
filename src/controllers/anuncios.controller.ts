...
const crearAnuncio = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const anuncio = new AnunciosModel({
      ...restoBody,
      identificacion,
      idEdificio,
    });
...
const getAnuncios = async (req: Request, res: Response) => {
  try {
    const anuncios = await AnunciosModel.find().populate(
      "idEdificio",
      "direccion identificadorEdificio"
    );
    res.status(200).json({
      ok: true,
      anuncios,
...
const getAnuncioById = async (req: Request, res: Response) => {
  ...
  try {
    const anuncio = await AnunciosModel.findById(id).populate(
      "idEdificio",
      "direccion identificadorEdificio"
    );
    if (!anuncio) {
...
const actualizarAnuncio = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (identificacion && identificacion !== anuncioDB.identificacion) {
      const existeIdentificacion = await AnunciosModel.findOne({
        identificacion,
      });
    ...
    const anuncioActualizado = await AnunciosModel.findByIdAndUpdate(
      id,
      campos,
      { new: true }
    );
